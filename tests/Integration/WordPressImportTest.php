...
final class WordPressImportTest extends TestCase
{
    ...
    public function testScanReportsContentWithoutWriting(): void
    {
        ...
        $this->assertSame(2, $scan->counts['attachment']);
    ...
    public function testImportCreatesPostsPagesAndTerms(): void
    {
        ...
        $this->assertSame('old.example.com:11', $classic['external_id']);
    ...
    public function testInternalLinksArePointedAtTheImportedCopies(): void
    {
        $this->runImport(sourceSiteUrl: 'https://old.example.com');

        $body = json_decode((string) $this->postBySlug('hello-classic')['body'], true);

        $this->assertSame(
            '/blog/second-post',
            $this->firstLinkHref($body['content']),
            'A link to another imported post becomes a local link'
        );
    }

    public function testInternalLinksAreLeftAloneWhenRewritingIsOff(): void
    {
        $importId = $this->service->create(
            'wordpress',
            self::FIXTURE,
            new ImportOptions(sourceSiteUrl: 'https://old.example.com', rewriteLinks: false)
        );
        do {
            $result = $this->service->advance($importId);
        } while (!$result['done']);
        ...
        $this->assertSame(
            'https://old.example.com/2019/06/second-post/',
            $this->firstLinkHref($body['content'])
        );
    }

    public function testRawHtmlIsDroppedAndCountedForUsersWhoMayNotPublishIt(): void
    {
        $importId = $this->service->create(
            'wordpress',
            self::FIXTURE,
            new ImportOptions(allowRawHtml: false)
        );
        do {
            $result = $this->service->advance($importId);
        } while (!$result['done']);
        ...
        $types = array_column(
            json_decode((string) $this->postBySlug('hello-classic')['body'], true)['content'],
            'type'
        );

        $this->assertSame(['paragraph', 'paragraph'], $types, 'The table block is gone');
        $this->assertSame(
            1,
            json_decode((string) $this->service->find($importId)['summary'], true)['dropped_raw_html'],
            'Dropping must be counted, never silent'
        );
    }

    public function testRedirectMapPairsEveryOldUrlWithItsNewLocation(): void
    {
        $importId = $this->runImport();
        ...
        $this->assertSame('/blog/hello-classic', $map['/2019/05/hello-classic/']);
        $this->assertSame('/about', $map['/about/']);
        $this->assertSame('/team', $map['/about/team/']);
        $this->assertSame(
            '/blog/scheduled-announcement',
            $map['/?p=50'],
            'A ?p= permalink keeps its query — its path alone is the home page'
        );
        $this->assertArrayNotHasKey('/', $map, 'Never emit a rule that redirects the home page');
    }

    public function testFeaturedImageResolvesEvenThoughTheAssetIsListedAfterThePost(): void
    {
        $this->runImport();

        $postId = $this->postBySlug('second-post')['id'];

        $stmt = $this->pdo->prepare("SELECT og_image_id FROM seo_meta WHERE target_type = 'post' AND target_id = ?");
        $stmt->execute([$postId]);
        $ogImageId = $stmt->fetchColumn();

        $this->assertNotFalse($ogImageId, 'The post should have picked up a featured image');

        $stmt = $this->pdo->prepare('SELECT external_id, source_url, status FROM media WHERE id = ?');
        $stmt->execute([$ogImageId]);
        $media = $stmt->fetch();

        $this->assertSame('old.example.com:40', $media['external_id']);
        $this->assertSame('https://old.example.com/wp-content/uploads/2019/06/photo.jpg', $media['source_url']);
        $this->assertSame('pending', $media['status']);
    }

    public function testFeaturedImageIsTheSameRowAsTheSameImageInTheBody(): void
    {
        $this->runImport();

        // The body image and the featured image are the same file, so they
        // must be one row and one download.
        $this->assertSame(1, (int) $this->pdo->query('SELECT COUNT(*) FROM media')->fetchColumn());
        $this->assertSame(
            1,
            (int) $this->pdo->query("SELECT COUNT(*) FROM jobs WHERE handler = 'import.media'")->fetchColumn()
        );
    }

    public function testDanglingFeaturedReferenceIsCountedRatherThanIgnored(): void
    {
        $importId = $this->runImport();

        $summary = json_decode((string) $this->service->find($importId)['summary'], true);

        $this->assertSame(1, $summary['featured_resolved']);
        $this->assertSame(1, $summary['featured_unresolved'], 'Attachment 999 is not in the export');
        $this->assertContains(
            '1 featured image(s) point at assets that are not in this export.',
            $summary['warnings']
        );
    }

    public function testUnreferencedAttachmentsAreNotDownloaded(): void
    {
        $this->runImport();
        ...
        $this->assertNotContains(
            'https://old.example.com/wp-content/uploads/2019/07/unused.jpg',
            $urls,
            'An export lists the whole media library; only what is used should come across'
        );
    }

    public function testExternalIdsAreQualifiedBySourceSiteSoTwoSitesCannotCollide(): void
    {
        $this->runImport();

        $this->assertSame('old.example.com:11', $this->postBySlug('hello-classic')['external_id']);
        $this->assertSame(
            'old.example.com:20',
            $this->postBySlug('team')['external_parent_id'],
            'Parent references use the same qualified form'
        );
    }

    public function testImportedFilesKeepTheSourceYearAndMonth(): void
    {
        $this->runImport();

        $path = (string) $this->pdo->query('SELECT path FROM media LIMIT 1')->fetchColumn();

        $this->assertStringStartsWith(
            'import/2019/06/',
            $path,
            'An asset uploaded in 2019 belongs under 2019, whatever today is'
        );
    }
    ...
    private function runImport(bool $asDraft = false, ?string $sourceSiteUrl = null): string
    {
        $importId = $this->service->create(
            'wordpress',
            self::FIXTURE,
            new ImportOptions(asDraft: $asDraft, sourceSiteUrl: $sourceSiteUrl)
        );
        do {
            $result = $this->service->advance($importId);
        } while (!$result['done']);
     ...
     */
    private function firstLinkHref(array $nodes): ?string
    {
